...
def write_icon_data_from_mesh_object():
    filename = ""
    objs = [obj for obj in bpy.data.objects if obj and obj.type == 'MESH' and obj.data.vertex_colors and not obj.name.rpartition(".")[2].isdigit()]
    objs.sort(key=lambda a: a[0])
    ...
    for obj in objs:
        ...
        with open(filename, 'wb') as write_file:
            write_mesh_to_py(write_file, obj, objects_children.get(obj, []))
    return filename
